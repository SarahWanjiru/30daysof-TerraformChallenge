...
# High CPU alarm — triggers scale-out and notifies ops team
resource "aws_cloudwatch_metric_alarm" "high_cpu" {
  ...
  statistic           = "Average"
  threshold           = 70
  alarm_description   = "CPU utilization exceeded 70% for 4 minutes on ${var.cluster_name}"
...
# Low CPU alarm — detects idle cluster, signals scale-in opportunity
resource "aws_cloudwatch_metric_alarm" "low_cpu" {
  ...
  statistic           = "Average"
  threshold           = 10
  alarm_description   = "CPU utilization below 10% for 4 minutes on ${var.cluster_name} — cluster may be over-provisioned"
  alarm_actions       = [aws_sns_topic.alerts.arn]
  ...
  tags = merge(local.common_tags, { Name = "${var.cluster_name}-low-cpu-alarm" })
}
