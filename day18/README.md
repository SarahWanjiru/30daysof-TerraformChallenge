...
![alt text](<Screenshot from 2026-04-06 20-23-43.png>)

 PR page showing unit tests passing
...
Real infrastructure deployed, ALB hit, HTTP assertion passed, 11 resources destroyed by
`defer terraform.Destroy`. Total time: 9 minutes 32 seconds.

The ASG took 5 minutes 55 seconds to destroy — AWS waits for all instances to terminate
before removing the ASG. This is normal behaviour.

**First attempt failed** with `missing go.sum entry`. Go requires a `go.sum` file recording
cryptographic hashes of all dependencies. Fixed by running `go mod tidy` locally and
committing the generated `go.sum` file.
