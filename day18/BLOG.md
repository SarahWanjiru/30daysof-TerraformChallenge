...
6. The Errors and Fixes

Error 1 — Missing go.sum

After the first merge, integration tests failed:
...
Fix: run go mod tidy locally and commit the generated go.sum file.

📸 Screenshot here — the missing go.sum error in GitHub Actions
Caption: First integration test run — go.sum not committed, Go cannot verify dependencies

After the fix, the second merge ran the integration test successfully:

--- PASS: TestWebserverClusterIntegration (539.80s)
PASS
ok  	test	540.057s

Real infrastructure deployed. ALB hit. HTTP assertion passed. 11 resources destroyed by defer terraform.Destroy. Total time: 9 minutes 32 seconds.

The ASG took 5 minutes 55 seconds to destroy — AWS waits for all instances to terminate before removing the ASG. This is normal.

📸 Screenshot here — the PASS result in GitHub Actions showing 539.80s
Caption: Integration test passing — real infrastructure deployed, tested, and destroyed automatically
